...
{
	ui_send_button_event(SCREEN_MEG, code, direct);
	switch (code)
	{
...
{	
	...
	ui_create_pct_title(ui_pct_screen, SCREEN_MEG);
	...
	lv_obj_set_pos(obj, x, y); x += button_large_width + gap * 5; ui_pct_keyboard[KEYBOARD_DIAG] = obj;
	...
	lv_obj_set_pos(obj, x, y); x += button_large_width + gap * 5; ui_pct_keyboard[KEYBOARD_BACK] = obj;
	...
	lv_obj_set_pos(obj, x, y); x += button_large_width + gap * 5; ui_pct_keyboard[KEYBOARD_ENTER] = obj;
