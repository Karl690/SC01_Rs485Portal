...
bool load_configuration()
{
	...
	if (!systemconfig.serial1.baud) systemconfig.serial1.baud = SERIAL_BAUD_RATE;
	if (!serial_valid_pins(systemconfig.serial1.rx_pin)) systemconfig.serial1.rx_pin = SERIAL_UART1_RXD_PIN;
	if (!serial_valid_pins(systemconfig.serial1.tx_pin)) systemconfig.serial1.tx_pin = SERIAL_UART1_TXD_PIN;
	
	if (!systemconfig.serial2.baud) systemconfig.serial2.baud = SERIAL_BAUD_RATE;
	if (!serial_valid_pins(systemconfig.serial2.rx_pin)) systemconfig.serial2.rx_pin = SERIAL_UART2_RXD_PIN;
	if (!serial_valid_pins(systemconfig.serial2.tx_pin)) systemconfig.serial2.tx_pin = SERIAL_UART2_TXD_PIN;
	if (systemconfig.serial2.mode > 2) systemconfig.serial2.mode = 0;
	
	if (!systemconfig.secs.timerReload1) systemconfig.secs.timerReload1 = 3;
	if (!systemconfig.secs.timerReload2) systemconfig.secs.timerReload2 = 5;
